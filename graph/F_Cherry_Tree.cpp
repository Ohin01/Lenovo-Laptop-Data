...
void dfs(ll node, ll par, vector<set<ll>> &states)
{
    for (auto child: graph[node])
    {
        if (child == par) continue;

        dfs(child, node, states);
    }
    ...
    for (auto child: graph[node])
    {
        ...
        swap(temp, states[node]);
    }

    states[node].insert(1);
}
...
void solve()
{
    ...
    vector<set<ll>> states(n + 1);

    dfs(1, 0, states);

    auto it = states[1].find(0);
