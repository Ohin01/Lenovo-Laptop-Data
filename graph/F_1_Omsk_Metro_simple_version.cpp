...
void solve()
{
    ...
    vvll dp(n + 10, vll(2, 0)), state(n + 10, vll(2, 0));
    ...
    while (n--)
    {
        ...
        if (ch == '+')
        {
            ...
            state[node][0] = min(0LL, state[v][0] + x);
            state[node][1] = max(0LL, state[v][1] + x);

            dp[node][0] = min(dp[v][0], state[node][0]);
            dp[node][1] = max(dp[v][1], state[node][1]);
