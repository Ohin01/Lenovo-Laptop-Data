...
struct DSU {
    ...
    ll c, ans;
    DSU(ll n) : par(n + 1), rnk(n + 1, 0), sz(n + 1, 1), c(n), ans(0) {
...
void solve()
{
    ...
    vector<vector<pll>> inds(n + 1);

    FOR(i, 0, n - 1)
    {
        inds[abs(a[i] - a[i + 1])].pb({i, i + 1});
    }
    ...
    //     FOR(j, 0, inds[i].size())
    //     {
    //         cout << inds[i][j].ff << " " << inds[i][j].ss << endl;
    //     }
    // }
    ...
    FORR(i, n - 1, 1)
    {
        FOR(j, 0, inds[i].size())
        {
            dsu.merge(inds[i][j].ff, inds[i][j].ss);
        }
        ans[i] = dsu.ans;
    }
