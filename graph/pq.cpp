...
struct cmp {
    bool operator()(const Node &a, const Node &b) const {
        return a.first > b.first; // min-heap by .first (ll)
    }
};
...
void solve()
{
    ...
    q1.push({0, {1, 0}}); dist1[1][0] = 0;
    q2.push({0, {n, 0}}); dist2[n][0] = 0;

    vvb vis1(n + 1, vb(2, false)), vis2(n + 1, vb(2, false));

    while (!q1.empty())
    {
        pair<ll, pair<ll, ll>> front = q1.top();
        q1.pop();

        ll node = front.ss.ff;
        ll wt = front.ff;
        ll onhorse = front.ss.ss;

        int state = onhorse? 1:0;

        if (wt != dist1[node][state]) continue;
        ...
        for (auto child: graph[node])
        {
            ...
            if (dist1[node][state] + chwt < dist1[chnode][newstate])
            {
                ...
                q1.push({dist1[chnode][newstate], {chnode, onhorse}});
            }
    ...
    while (!q2.empty())
    {
        pair<ll, pair<ll, ll>> front = q2.top();
        q2.pop();
        ...
        if (wt != dist2[node][state]) continue;
        ...
        for (auto child: graph[node])
        {
            ...
            if (dist2[node][state] + chwt < dist2[chnode][newstate])
            {
                ...
                q2.push({dist2[chnode][newstate], {chnode, onhorse}});
            }
