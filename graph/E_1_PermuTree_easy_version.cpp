...
void dfs(ll node, ll par, vvll &graph, vll &childcount)
{
    for (auto child: graph[node])
    {
        if (child == par) continue;

        dfs(child, node, graph, childcount);
...
void solve()
{
    ...
    vll childcount(n + 1, 1);

    dfs(1, -1, graph, childcount);
    ...
    while (!q.empty())
    {
        ...
        for (auto child: graph[node])
        {
            ...
            tot += childcount[child];
            mx = max(mx, childcount[child]);
        }
