...
deque<set<int>> dfs(ll node, ll par)
{
    deque<set<int>> dq;
    dq.pb({id[str[node]]});
    ...
    for (auto child: graph[node])
    {
        ...
        auto chDq = dfs(child, node);

        ll diff = 1;

        if (dq.size() < chDq.size())
        {
            swap(dq, chDq);
            dq.push_front({});
            diff = 0;
        ...
        while (dq.size() < chDq.size() + 1) dq.push_back({});

        for (int i = 0; i < chDq.size(); i++)
        {
            if (i + diff >= dq.size())
            {
                dq.push_back(chDq[i]);
