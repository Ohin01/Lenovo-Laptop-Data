...
void dfs(ll node, ll par)
{
    ...
    if (cnt[node][color[node]] == mx[node])
    ans[node] += color[node];

    else if (cnt[node][color[node]] > mx[node])
    {
        ans[node] = color[node];
        mx[node] = cnt[node][color[node]];
    }
    ...
    for (auto child: graph[node])
    {
        ...
        {
            cnt[node][p.ff] += cnt[child][p.ff];

            if (cnt[node][p.ff] == mx[node])
            ans[node] += p.ff;

            else if (cnt[node][p.ff] > mx[node])
            {
                ans[node] = p.ff;
                mx[node] = cnt[node][p.ff];
            }
        }
    }
