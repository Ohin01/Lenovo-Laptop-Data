...
int main(void)
{
    ...
    FOR(i, 1, MX)
    {
        ...
        auto nxt = lower_bound(rows.begin(), rows.end(), i);

        if (*nxt != i)
        nxt--;

        ll cur = (nxt - rows.begin());
  
        ll diff = rows[cur] - rows[cur - 1];

        //cout << i << " " << cur << " " << diff << endl;

        ll count = 0;

        if (i - diff >= rows[cur - 1] && i - diff < rows[cur])
        {
            dp[i] += dp[i - diff];
            count++;
        }

        if (i - diff - 1 >= rows[cur - 1] && i - diff - 1 < rows[cur])
        {
            dp[i] += dp[i - diff - 1];
            count++;
        }
        ...
        if (cur >= 2 && count == 2)
        {
            ll diff2 = rows[cur - 1] - rows[cur - 2];

            //cout << i << " " << (i - diff - 1) - diff2 << endl;

            if ((i - diff - 1) - diff2 >= rows[cur - 2] && (i - diff - 1) - diff2 < rows[cur - 1])
            dp[i] -= dp[(i - diff - 1) - diff2];
        }
